...
import { LayoutMangaExtendedState } from '@screens/Home/screens/MangaLibrary/components/MangaLibraryRLV/MangaLibraryRLV.recycler';
import MangaList from '@components/MangaList';
import { renderItem, keyExtractor } from '../../GenericMangaList.flatlist';

const InfiniteMangaListRLV: React.FC<{ genre: string; source: string }> = (props) => {
...
const InfiniteMangaListFlatList: React.FC<{ genre: string; source: string }> = (props) => {
  const { genre, source } = props;
  const mangaSource = useMangaSource(source) as MangaHostWithFilters<{ Genres: ExclusiveInclusiveFilter<string> }>;
  const {
    state: [mangas],
  ...
  React.useEffect(() => {
    if (!loading && mangas) setMangaList(mangas);
  }, [mangas, loading]);
  if (ready) {
    if (loading || !mangas)
      return (
...
export default function (props: { genre: string; source: string }) {
  const useRecyclerListView = useSelector((state: AppState) => state.settings.advanced.useRecyclerListView);
  if (useRecyclerListView) return <InfiniteMangaListRLV {...props} />;
  return <InfiniteMangaListFlatList {...props} />;
}
